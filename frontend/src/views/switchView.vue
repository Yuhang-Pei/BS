<template>
  ...
  <navigate :current-choice="7"></navigate>
  <div class="main">
    ...
    <switch-statics :key="statics_key"></switch-statics>

    <category-room class="rooms" v-for="(room, room_id) in room_list" v-show="isShowRoom(room_id)" :room-id="room_id" type="switch"></category-room>
...
export default {
  name: "switchView",
  components: {SwitchStatics, CategoryRoom, Loading, Navigate, BackPicture},
  data() {
    return {
      loading: true,
  ...
  methods: {
    isShowRoom(room_id) {
      for (let switch_id in global_data.switch_list)
        if (global_data.switch_list[switch_id].room_id == room_id)
          return true;
      return false;
    },
    updateStatics() {
