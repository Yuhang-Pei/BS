...
<!--      <img class="sensorIcon" :src="require(status?'@/assets/furnitureIcon/sensor-on.svg':'@/assets/furnitureIcon/sensor-off.svg')">-->
      ...
      <div class="fade sensorValue" :class="{'hide':(!transition)&&selected}">
        <span class="temperatureValue">{{temperature}}</span><span class="temperatureUnit">℃&ensp;</span>
        <span class="humidityValue">{{humidity}}</span><span class="humidityUnit">%</span>
      </div>
...
.sensorButtonOn .sensorSubCaption {
  color: rgb(147, 143, 137);
  font-size: 12px;
}
