<template>
  <div class="lockButtonWrapper" @mousedown="onDrag">
    <div @click="selectLockButton" class="lockButton" :class="{lockButtonSelected:selected, lockButtonTransition:transition, lockButtonOn:status, lockButtonOff:!status}">
...
<!--      <img class="lockIcon" :src="require(status?'@/assets/furnitureIcon/lock-on.svg':'@/assets/furnitureIcon/lock-off.svg')">-->
      ...
      <h1 class="fade" :class="{'hide':transition||!selected}">{{lockName}}</h1>
      <h2 class="fade" :class="{'hide':transition||!selected}">{{(lockRoom) + ' | ' + (status?'Open':'Close')}}</h2>
      <svg class="fade closeButton" :class="{'hide':transition||!selected}" @click="deselectLockButton" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      ...
      <div class="fade lockButtonRoomLine" :class="{'hide':transition||!selected, 'translateY':transition||!selected}">
        <h3>房间</h3>
      ...
      <div class="fade lockButtonSwitchLine" :class="{'hide':transition||!selected, 'translateY':transition||!selected}">
        <h3>开关</h3>
        <Switch class="lockButtonSwitch" @click="changeLockStatus" :switch-status="status"></Switch>
      </div>
    </div>
  </div>
  <Mask :class="{'hide':!selected}" @click="deselectLockButton"></Mask>
</template>
...
export default {
  ...
  methods: {
    ...
    changeLockStatus() {
      this.$http.post('/lock/chagne_lock_state', {
        'lock_id': this.lockId,
    ...
    onDrag(e) {
      ...
      if (this.edit) {
        ...
        document.onmouseup = () => {
          ...
          global_data.lock_list[this.lockId].device_x = left;
          global_data.lock_list[this.lockId].device_y = top;
          this.$emit('changePosition', 1, this.lockId, left, top);
        }
      }
    },
    getRoomFocus() {
...
.lockButtonOn h1, .lockButtonOff h3 {
  color: rgb(5, 5, 5);
}
...
.lockButtonOff h1, .lockButtonOff h3 {
...
.lockButtonRoomLine, .lockButtonSwitchLine {
  position: relative;
  top: 63px;
  margin: 0 15px;
  height: 30px;
...
.lockButtonOn .roomValue {
  color: rgb(80, 80, 80);
}

.lockButtonOn .roomValue:hover {
  color: rgb(5, 5, 5);
}

.lockButtonOff .roomValue {
  color: rgb(220, 215, 205);
}

.lockButtonOff .roomValue:hover {
  color: white;
}
